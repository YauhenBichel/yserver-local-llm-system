...
| [llm-hops](https://github.com/YauhenBichel/llm-hops) | See every hop of a request through your local LLM system: a tracing server in one static binary, a live flow map, a waterfall per request, stats per hop and per model; follows the gateway's request log, or takes spans from the Python SDK | Flow 1 drawn live, for every request |
